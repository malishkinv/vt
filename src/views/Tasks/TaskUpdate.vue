<template lang="pug">
  div
    ...
    template(v-if='loaded')
      form(:class='$style.form' :style="{'background-color':bgColor}" @submit.prevent='updateTask')
        ...
        div(:class='$style.formRow')
          div(:class='$style.formButtons')
            ...
            button(:class='$style.formButton' :disabled='!validated' @click='updateTask') Сохранить
    template(v-else='')
...
export default {
  ...
  methods: {
    ...
    updateTask() {
      this.error = null;
      ApiService.put("/tasks/" + this.itemId, this.item).then(() => {
